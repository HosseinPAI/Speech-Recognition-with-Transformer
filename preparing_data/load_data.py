import argparse
import json
from preparing_data.data_loader import SpectrogramDataset, AudioDataLoader, BucketingSampler
...
# -----------------------------------------------------------------------------------------
# -----------------------------------------------------------------------------------------
def load_data(train_manifest_list, valid_manifest_list, test_manifest_list, batch_size=12):
    audio_conf = dict(sample_rate=16000, window_size=0.02, window_stride=0.01, window='hamming')
    PAD_CHAR = "¶"
    SOS_CHAR = "§"
    EOS_CHAR = "¤"
    ...
    for i in range(len(labels)):
        if labels[i] not in label2id:
            ...
            count += 1
    train_data = SpectrogramDataset(audio_conf, manifest_filepath_list=train_manifest_list, label2id=label2id,
                                    normalize=True, augment=False)
    # print('train_data ', train_data)
    train_sampler = BucketingSampler(train_data, batch_size=batch_size)
    # print('train_sampler: ', train_sampler)
    train_loader = AudioDataLoader(train_data, num_workers=0, batch_sampler=train_sampler)
    ...
    for i in range(len(valid_manifest_list)):
        valid_data = SpectrogramDataset(audio_conf, manifest_filepath_list=[valid_manifest_list[i]], label2id=label2id,
                                        normalize=True, augment=False)
        valid_loader = AudioDataLoader(valid_data, num_workers=0, batch_size=batch_size)
    ...
    for i in range(len(test_manifest_list)):
        test_data = SpectrogramDataset(audio_conf, manifest_filepath_list=[test_manifest_list[i]], label2id=label2id,
                                       normalize=True, augment=False)
        test_loader = AudioDataLoader(test_data, num_workers=0)
        test_loader_list.append(test_loader)

    print('>>> Create Loaders Done')
